...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 30-03-2013
 #PACKAGE: idoc-config
 #DESCRIPTION: Interactive Documents list
 #VERSION: 2.1beta
 #CHANGELOG:
 #TODO:
...
$ret = GShell("dynarc cat-info -ap `idoc` -id `".($_REQUEST['cat'] ? $_REQUEST['cat'] : $_REQUEST['parent'])."`");
$catInfo = $ret['outarr'];

/* Detect icon */
if(file_exists($_BASE_PATH."iDoc/icons/".strtolower($catInfo['tag']).".png"))
 $icon = strtolower($catInfo['tag']).".png";
else
 $icon = "other.png";

?>
<link rel="stylesheet" href="<?php echo $_ABSOLUTE_URL; ?>iDoc/idoclist.css" type="text/css" />

<div class="strappo">&nbsp;</div>
<table width='100%' cellspacing='0' cellpadding='4' border='0'>
<tr><td width='50'>&nbsp;</td>
	<td valign='middle'><span class='gray24'><?php echo $catInfo['name']; ?></span><?php
	if($_REQUEST['copy'])
	...
	<td align='right'>
		<a href='#' onclick='newDoc()' title="Nuovo documento"><img src="<?php echo $_ABSOLUTE_URL; ?>iDoc/img/btn-add.png" border='0'/></a>
		<a href='#' onclick='idocImport()' title="Importa da file"><img src="<?php echo $_ABSOLUTE_URL; ?>iDoc/img/btn-import.png" border='0'/></a>
	</td></tr>
</table>
...
<div class="elementstable-container" id="elementstable-container">
 ...
 <tr><th width='50' style="border-right:1px solid #fcd1bb;">&nbsp;</th>
  ...
  <?php
  $ret = GShell("dynarc item-list -ap `idoc` -cat `".$catInfo['id']."` -get thumbdata");
  ...
  for($c=0; $c < count($list); $c++)
  {
   ...
   echo "<div class='modelinfo'><a href='".$_ABSOLUTE_URL."iDoc/edit.php?id=".$itm['id']."' target='GJKIDOC-".$itm['id']."' class='title' id='idoc-"
	.$itm['id']."-title'>".$itm['name']."</a>";
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `idoc` -cat `<?php echo $catInfo['id']; ?>` -name `"+nm+"`"); 
}
...
function copySelected()
{
 ...
 document.location.href = "?vis=category&parent=<?php echo $catInfo['parent_id']; ?>&cat=<?php echo $catInfo['id']; ?>&copy="+q.substr(1);
}
...
function pasteHere(str)
{
 var x = str.split(",");
 var sh = new GShell();
 sh.OnFinish = function(){document.location.href="?vis=category&parent=<?php echo $catInfo['parent_id']; ?>&cat=<?php echo $catInfo['id']; ?>";}
 for(var c=0; c < x.length; c++)
  sh.sendCommand("dynarc item-copy -ap `idoc` -cat `<?php echo $catInfo['id']; ?>` -id `"+x[c]+"`");
}
...
function exportSelected()
{
 ...
 if(SELECTED_ROWS.length == 1)
  title = document.getElementById("idoc-"+SELECTED_ROWS[0].id+"-title").innerHTML;
 ...
 sh.sendCommand("dynarc export -ap `idoc` -f `"+title+"`"+q);
}
...
function idocImport()
{
 var sh = new GShell();
 sh.OnOutput = function(){document.location.reload();}
 sh.sendCommand("gframe -f dynarc.import -params `ap=idoc&cat=<?php echo $catInfo['id']; ?>`");
}
</script>
<?php
