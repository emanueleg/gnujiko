...
?>
<link rel="stylesheet" href="<?php echo $_ABSOLUTE_URL; ?>iDoc/vis-trash.css" type="text/css" />
...
<tr><td width='50'>&nbsp;</td>
	...
	<td align='right'><a href='#' onclick='emptyTrash()'><img src="<?php echo $_ABSOLUTE_URL; ?>iDoc/img/btn-trash-empty.png" border='0' title="Svuota cestino"/></a></td></tr>
</table>
...
<div class="elementstable-container" id="elementstable-container">
 <table width='100%' cellspacing='0' cellpadding='0' border='0' class='elementstable' id='elementstable'>
  ...
  <?php
  $ret = GShell("dynarc item-list -ap `idoc` --all-cat -where `trash=1` --include-trash -get thumbdata");
...
{
 if(!confirm("Sei sicuro di voler eliminare definitivamente i documenti selezionati?"))
  return;
 ...
 sh.sendCommand("dynarc trash remove -ap `idoc`"+q);
}
...
function restoreSelected()
{
 ...
 for(var c=0; c < SELECTED_ROWS.length; c++)
  q+= " -id `"+SELECTED_ROWS[c].id+"`";
 sh.sendCommand("dynarc trash restore -ap `idoc`"+q);
}
...
function emptyTrash()
{
 ...
 sh.sendCommand("dynarc trash empty -ap idoc");
}
</script>
<?php
