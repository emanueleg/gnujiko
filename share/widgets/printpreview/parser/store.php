...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 13-09-2012
 #PACKAGE: store
 #DESCRIPTION: Official Gnujiko Store parser for print preview.
 #VERSION: 2.0beta
 #CHANGELOG:
 #TODO:
...
function storeInsertRows(itemList)
{
 ...
 for(var c=start; c < itemList.length; c++)
 {
  ...
  for(var i=0; i < tb.rows[0].cells.length; i++)
  {
   ...
   var sq = (STORE_ID != '0') ? itemList[c]['store_'+STORE_ID+'_qty'] : itemList[c]['storeqty'];
   var ava = parseFloat(itemList[c]['storeqty'])-parseFloat(itemList[c]['booked']);
   var inc = parseFloat(itemList[c]['incoming']);
   if(ava < 0)
	inc+= ava;

   switch(tb.rows[0].cells[i].id)
   {
    case 'storeqty' : value = sq; break;
	case 'available' : {
	 	 value = ava;
	     if(inc > 0)
	      value+= "&nbsp;<b>+"+inc+"</b>";
