...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: New credit form for Petty Cash Book.
 #VERSION: 2.0beta
 #CHANGELOG:
 #TODO:
...
?>
<html><head><meta http-equiv="content-type" content="text/html; charset=UTF-8"><title>New Credit</title>
<?php
...
$form = new GForm("Nuova entrata", "MB_OK|MB_ABORT", "simpleform", "default", "orange", 420, 320);
...
function OnFormSubmit()
{
 ...
 sh.sendCommand("dynarc new-item -ap `pettycashbook` -ctime `"+date+"`"+(catId ? " -cat `"+catId+"`" : "")+" -name `"+description+"` -extset `pettycashbook.resin='"+resId+"',in='"+amount+"',docap='"+docAp+"',docid='"+docId+"',docref='"+docRef+"',"+(subjectId ? "subjectid='"+subjectId+"'" : "subject='''"+subjectName+"'''")+"`");
}
