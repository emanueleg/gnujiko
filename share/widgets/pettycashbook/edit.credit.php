...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: Edit credit form for Petty Cash Book.
 #VERSION: 2.0beta
 #CHANGELOG:
 #TODO:
...
?>
<html><head><meta http-equiv="content-type" content="text/html; charset=UTF-8"><title>Edit Credit</title>
<?php
...
$form = new GForm("Modifica entrata", "MB_OK|MB_ABORT", "simpleform", "default", "orange", 420, 320);
$form->Begin($_ABSOLUTE_URL."share/widgets/pettycashbook/img/incomes.png");
...
function OnFormSubmit()
{
 ...
 sh.sendCommand("dynarc edit-item -ap `<?php echo $archivePrefix; ?>` -id `<?php echo $itemInfo['id']; ?>` -ctime `"+date+"`"+(catId ? " -cat `"+catId+"`" : "")+" -name `"+description+"` -extset `pettycashbook.resin='"+resId+"',in='"+amount+"',docap='"+docAp+"',docid='"+docId+"',docref='"+docRef+"',"+(subjectId ? "subjectid='"+subjectId+"'" : "subject='''"+subjectName+"'''")+"`");
}
