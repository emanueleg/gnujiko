...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 26-10-2014
 #PACKAGE: gcommercialdocs-receipts
...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['RECEIPTS'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['RECEIPTS'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "Ricevute Fisc. cestinate"; else echo "Ricevute Fiscali"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea una nuova Ricevuta Fiscale</span></li>";
		 ?>
	</ul></td>
	<td>	 
...
if(!$catInfo)
{
 $ret = GShell("dynarc cat-info -ap `commercialdocs` -tag receipts",$_REQUEST['sessid'],$_REQUEST['shellid']);
 if(!$ret['error'])
  $catInfo = $ret['outarr'];
}

$ret = GShell("dynarc item-list -ap `commercialdocs` -into receipts -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -into receipts -where `status<10` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 <li class="<?php echo (!$_REQUEST['show']) ? 'selected' : 'first'; ?>"><div><span class='title' onclick="showPage('',true)">Tutte le ricevute</span><br/><em>Tot. doc:</em><span class="gray right"><?php echo $numOfDocuments; ?></span></div></li>
	 ...
	 <?php
	 /* SOTTOCATEGORIE */
	 $ret = GShell("dynarc cat-list -ap commercialdocs -pt `receipts` --get-total-items-count",$_REQUEST['sessid'],$_REQUEST['shellid']);
	 if(!$ret['error'])
	 {
	  $list = $ret['outarr'];
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['RECEIPTS'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['RECEIPTS'] : array(
	 		0 => array("tag"=>"name", "title"=>"Ricevuta", "width"=>190, "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` ".(($_REQUEST['catid'] && ($_REQUEST['show'] == "category")) ? "-cat `".$catInfo['id']."`" : "-into receipts")." -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `".$orderBy."`".($limit ? " -limit ".$limit : "")." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
	 ...
	 if(($lastMonth != date('n',$itm['ctime'])) || ($lastYear != date('Y',$itm['ctime'])))
	 {
	  ...
	  if($_REQUEST['show'] == "ungroupped")
	  {
	   if($lastSubjectName && ($lastSubjectName != $itm['subject_name']))
	    echo "<tr class='label2'><td colspan='100'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/lu.gif' style='margin-left:16px;'/> <i>Raggruppa tutte le ricevute selezionate per ".$lastSubjectName."</i><a href='#' onclick='groupDocs(this,true)' class='roundbtn'>Raggruppa tutte</a> <a href='#' class='roundbtn' onclick='groupDocs(this)'>Raggruppa selezionate</a></td></tr><tr class='label2'><td colspan='100'>&nbsp;</td></tr>";
  ...
  <tr><th class="blue" rowspan="2" style="text-align: left;">
	   <div class="btn-filter" id="btn-filter" filterval="<?php echo $_REQUEST['ft'] ? $_REQUEST['ft'] : 'all'; ?>"><span><?php
		...
		<ul class='submenu' id="filter-list">
	 	 <li onclick="filterChange('all',this)">Tutte</li>
		 <li class='separator'>&nbsp;</li>
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` "+((show == "category") ? "-cat `"+CAT_ID+"`" : "-into receipts")+" -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -cat `"+CAT_ID+"` -group commdocs-receipts");
}
