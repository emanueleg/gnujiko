...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['PURCHASEINVOICES'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['PURCHASEINVOICES'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "Fatture d&lsquo;acquisto cestinate"; else echo "Fatture d&lsquo;acquisto"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea una nuova fattura d&lsquo;acquisto</span></li>";
		 ?>
	</ul></td>
	<td>	 
...
<?php
$ret = GShell("dynarc cat-info -ap `commercialdocs` -tag purchaseinvoices",$_REQUEST['sessid'],$_REQUEST['shellid']);
$catInfo = $ret['outarr'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct purchaseinvoices -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct purchaseinvoices -where `status<10` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$toBePaid = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct purchaseinvoices -where `status=10` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$paid = $ret['outarr']['count'];
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 <li class="<?php echo (!$_REQUEST['show']) ? 'selected' : 'first'; ?>"><div><span class='title' onclick="showPage()">Tutte</span><br/><em>Tot. doc:</em><span class="gray right"><?php echo $numOfDocuments; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'tobepaid') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('tobepaid')">Da pagare</span><br/><em>N.</em><span class="gray right"><?php echo $toBePaid; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'paid') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('paid')">Pagate</span><br/><em>N.</em><span class="gray right"><?php echo $paid; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'trash') ? 'selected' : 'last'; ?>"><div><span class='title' onclick="showPage('trash')">Cestinate</span><br/><em>N.</em><span class="gray right"><?php echo count($trashInfo['items']); ?></span></div></li>
	</ul>
	</td><td valign='middle' align='right' style="border-bottom: 1px solid #dedede;border-collapse:collapse;">
	<input type='text' class='searchinput' id='subject' emptyvalue="Cerca per fornitore" style="width:200px;margin-right:5px;" value="<?php echo $_REQUEST['subjectname']; ?>"/>
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['PURCHASEINVOICES'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['PURCHASEINVOICES'] : array(
	 		0 => array("tag"=>"name", "title"=>"Fattura d&lsquo;acquisto", "width"=>190, "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` -ct purchaseinvoices -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC")."` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` -ct purchaseinvoices -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -ct purchaseinvoices -group commdocs-purchaseinvoices");
}
...
function emptyTrash()
{
 ...
 sh.sendCommand("dynarc trash empty -ap commercialdocs -ct purchaseinvoices");
}
