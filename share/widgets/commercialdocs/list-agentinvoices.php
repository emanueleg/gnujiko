...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: Agent Invoice list
 #VERSION: 2.5beta
...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['AGENTINVOICES'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['AGENTINVOICES'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "Fatture agente cestinate"; else echo "Fatture agente"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea una nuova fattura agente</span></li>";
		 ?>
	</ul></td>
	<td>	 
...
<?php
$ret = GShell("dynarc cat-info -ap `commercialdocs` -tag agentinvoices",$_REQUEST['sessid'],$_REQUEST['shellid']);
$catInfo = $ret['outarr'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct agentinvoices -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct agentinvoices -where `status=0` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$toBeEmit = $ret['outarr']['count'];
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 ...
	 <li class="<?php echo ($_REQUEST['show'] == 'tobeemit') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('tobeemit')">Da emettere</span><br/><em>N.</em><span class="gray right"><?php echo $toBeEmit; ?></span></div></li>
	...
	<input type='text' class='searchinput' id='subject' emptyvalue="Cerca per agente" style="width:200px;margin-right:5px;" value="<?php echo $_REQUEST['subjectname']; ?>"/>
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['AGENTINVOICES'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['AGENTINVOICES'] : array(
	 		0 => array("tag"=>"name", "title"=>"Fattura agente", "width"=>190, "style"=>"text-align:left"),
			1 => array("tag"=>"subject_code", "title"=>"Cod.", "width"=>50, "style"=>"text-align:left"),
			2 => array("tag"=>"subject_name", "title"=>"Agente", "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` -ct agentinvoices -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC")."` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` -ct agentinvoices -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -ct agentinvoices -group commdocs-agentinvoices");
}
...
function emptyTrash()
{
 ...
 sh.sendCommand("dynarc trash empty -ap commercialdocs -ct agentinvoices");
}
