...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['DEBITSNOTE'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['DEBITSNOTE'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "Note di debito cestinate"; else echo "Note di debito"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea una nuova nota di debito</span></li>";
		 ?>
	</ul></td>
	<td>	 
...
<?php
$ret = GShell("dynarc cat-info -ap `commercialdocs` -tag debitsnote",$_REQUEST['sessid'],$_REQUEST['shellid']);
$catInfo = $ret['outarr'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct debitsnote -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct debitsnote -where `status<10` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['DEBITSNOTE'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['DEBITSNOTE'] : array(
	 		0 => array("tag"=>"name", "title"=>"Nota di debito", "width"=>190, "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` -ct debitsnote -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC")."` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` -ct debitsnote -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -ct debitsnote -group commdocs-debitsnote");
}
...
function emptyTrash()
{
 ...
 sh.sendCommand("dynarc trash empty -ap commercialdocs -ct debitsnote");
}
