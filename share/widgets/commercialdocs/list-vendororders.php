...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['VENDORORDERS'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['VENDORORDERS'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "Ordini fornitore cestinati"; else echo "Ordini fornitore"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea un nuovo ordine fornitore</span></li>";
		 ?>
	</ul></td>
	<td>	 
...
<?php
$ret = GShell("dynarc cat-info -ap `commercialdocs` -tag vendororders",$_REQUEST['sessid'],$_REQUEST['shellid']);
$catInfo = $ret['outarr'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct vendororders -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct vendororders -where `status=0` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfOpenedOrders = $ret['outarr']['count'];
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 <li class="<?php echo (!$_REQUEST['show']) ? 'selected' : 'first'; ?>"><div><span class='title' onclick="showPage()">Tutti</span><br/><em>Tot. doc:</em><span class="gray right"><?php echo $numOfDocuments; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'opened') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('opened')">Aperti</span><br/><em>N.</em><span class="gray right"><?php echo $numOfOpenedOrders; ?></span></div></li>
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['VENDORORDERS'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['VENDORORDERS'] : array(
	 		0 => array("tag"=>"name", "title"=>"Ordine fornitore", "width"=>190, "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` -ct vendororders -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC")."` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` -ct vendororders -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -ct vendororders -group vendororders");
}
...
function emptyTrash()
{
 ...
 sh.sendCommand("dynarc trash empty -ap commercialdocs -ct vendororders");
}
