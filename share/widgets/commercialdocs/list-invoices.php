...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #CHANGELOG: 26-10-2014 : Sostituito total con tot_netpay
			 ...
			 08-08-2013 : Bug fix nei totali. (non calcolava le ritenute d'acconto, ecc...)
			 13-01-2013 : Bug fix in assign group at every new items.
			 01-09-2012 : Rimosso temporaneamente le tab "Da incassare questo mese" e "Ri.Ba. da emettere" da fare in seguito.
...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['INVOICES'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['INVOICES'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "Fatture cestinate"; else echo "Elenco delle fatture"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea una nuova fattura</span></li>";
		 ?>
	</ul></td>
	<td>	 
	...
	<input type='text' class='searchinput' id='subject' placeholder="Cerca per cliente" style="width:300px;margin-right:5px;" value="<?php echo $_REQUEST['subjectname']; ?>" onchange="updateSearch()"/>
    </td>
...
if(!$catInfo)
{
 $ret = GShell("dynarc cat-info -ap `commercialdocs` -tag invoices",$_REQUEST['sessid'],$_REQUEST['shellid']);
 if(!$ret['error'])
  $catInfo = $ret['outarr'];
}

$ret = GShell("dynarc item-list -ap `commercialdocs` -into invoices -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -into invoices -where `status<10` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$toBePaid = $ret['outarr']['count'];
...
/* TODO: ricavare le fatture da incassare il mese corrente ed i riba da emettere */
$toBePaidThisMonth = 0;
$numOfRiBa = 0;
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 <li class="<?php echo (!$_REQUEST['show']) ? 'selected' : 'first'; ?>"><div><span class='title' onclick="showPage('',true)">Tutte</span><br/><em>Tot. doc:</em><span class="gray right"><?php echo $numOfDocuments; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'tobepaid') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('tobepaid',true)">Da pagare</span><br/><em>N.</em><span class="gray right"><?php echo $toBePaid; ?></span></div></li>

	 <?php
	 /* SOTTOCATEGORIE */
	 $ret = GShell("dynarc cat-list -ap commercialdocs -pt `invoices` --get-total-items-count --order-by 'name ASC'",$_REQUEST['sessid'],$_REQUEST['shellid']);
	 if(!$ret['error'])
	 {
	  $list = $ret['outarr'];
	 ...
	 <!-- <li class="<?php echo ($_REQUEST['show'] == 'tobepaidthismonth') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('tobepaidthismonth')">Da incassare questo mese</span><br/><em>N.</em><span class="gray right"><?php echo $toBePaidThisMonth; ?></span></div></li> -->
	 <!-- <li class="<?php echo ($_REQUEST['show'] == 'riba') ? 'selected' : 'last'; ?>"><div><span class='title' onclick="showPage('riba')">Ri.Ba. da emettere</span><br/><em>N.</em><span class="gray right"><?php echo $numOfRiBa; ?></span></div></li> -->
	 <li class="<?php echo ($_REQUEST['show'] == 'trash') ? 'selected' : 'last'; ?>"><div><span class='title' onclick="showPage('trash',true)">Cestinate</span><br/><em>N.</em><span class="gray right"><?php echo count($trashInfo['items']); ?></span></div></li>
	</ul>
	</td></tr>
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['INVOICES'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['INVOICES'] : array(
	 		0 => array("tag"=>"name", "title"=>"Fattura", "width"=>190, "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` ".(($_REQUEST['catid'] && ($_REQUEST['show'] == "category")) ? "-cat `".$catInfo['id']."`" : "-into invoices")." -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC")."` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
	...
	for($c=0; $c < count($list); $c++)
	{
	 ...
	 $subtotAmount+= $itm['amount'];
	 $subtotVat+= $itm['vat'];
	 $subtotNetPay+= $itm['tot_netpay'];
	}
	?>
	</table>
...
<table class="docfooter-results" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:10px;">
  ...
  <tr>
	  <td class="blue" id="doctot-amount"><em>&euro;</em><?php echo number_format($subtotAmount,2,',','.'); ?></td>
	  <td class="blue" id="doctot-vat"><em>&euro;</em><?php echo number_format($subtotVat,2,',','.'); ?></td>
	  <td class="green" id="doctot-total"><em>&euro;</em><?php echo number_format($subtotNetPay,2,',','.'); ?></td>
  </tr>
</table>
...
function nextPage()
{
 ...
 sh.OnOutput = function(o,a){
	 var totalAmount = parseCurrency(document.getElementById('doctot-amount').innerHTML.substr(10));
	 var totalVat = parseCurrency(document.getElementById('doctot-vat').innerHTML.substr(10));
	 var totalNetPay = parseCurrency(document.getElementById('doctot-total').innerHTML.substr(10));
	 ...
	 if(a && a['items'])
	 {
	  ...
	  for(var c=0; c < a['items'].length; c++)
	  {
	   ...
	   totalVat+= parseFloat(itm['vat']);
	   totalNetPay+= parseFloat(itm['tot_netpay']);
	  }
	  document.getElementById('doctot-amount').innerHTML = "<em>&euro;</em>"+formatCurrency(totalAmount,2);
	  document.getElementById('doctot-vat').innerHTML = "<em>&euro;</em>"+formatCurrency(totalVat,2);
	  document.getElementById('doctot-total').innerHTML = "<em>&euro;</em>"+formatCurrency(totalNetPay,2);
	 }
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` "+((show == "category") ? "-cat `"+CAT_ID+"`" : "-into invoices")+" -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -cat `"+CAT_ID+"` -group commdocs-invoices");
}
