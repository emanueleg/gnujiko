...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: D.D.T. Vendor list
 #VERSION: 2.4beta
 #CHANGELOG: 26-10-2014 : Sostituito total con tot_netpay
			 02-06-2014 : Sostituito -ct vendors con -into vendors
			 27-05-2014 : Raggruppamento DDT fornitore.
			 26-05-2014 : Aggiornato col nuovo sistema di gestione colonne
 #TODO: 
...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['DDTIN'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['DDTIN'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "DDT cestinati"; else echo "Elenco D.D.T. Fornitore"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea un nuovo D.D.T.</span></li>";
		 ?>
	</ul></td>
	<td>	 
	  ...
	  <li class='blue' id='selectionmenu' style='visibility:hidden;'><span><img src="<?php echo $_ABSOLUTE_URL; ?>var/templates/basicapp/img/checkbox.png" border='0'/>Selezionati</span>
		<ul class="submenu">
		 ...
		 else
		 {
		  ...
		  if(!$_REQUEST['show'] || ($_REQUEST['show'] == "ungroupped"))
		   echo "<li onclick='groupSelected()'>Raggruppa selezionati</li>";
		 }
		 ?>
	...
	<input type='text' class='searchinput' id='subject' placeholder="Cerca per fornitore" style="width:260px;margin-right:5px;" value="<?php echo $_REQUEST['subjectname']; ?>" onchange="updateSearch()"/>
    </td>
...
if(!$catInfo)
{
 $ret = GShell("dynarc cat-info -ap `commercialdocs` -tag DDTIN",$_REQUEST['sessid'],$_REQUEST['shellid']);
 if(!$ret['error'])
  $catInfo = $ret['outarr'];
}

$ret = GShell("dynarc item-list -ap `commercialdocs` -into ddtin -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct ddtin -where `conv_doc_id=0 AND group_doc_id=0` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfUngrouppedDDT = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct ddtin -where `status=9` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfGrouppedDDT = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct ddtin -where `tag='INTERNAL'` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfIntDocs = $ret['outarr']['count'];
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 <li class="<?php echo (!$_REQUEST['show']) ? 'selected' : 'first'; ?>"><div><span class='title' onclick="showPage('',true)">Tutti i D.D.T.</span><br/><em>Tot. doc:</em><span class="gray right"><?php echo $numOfDocuments; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'ungroupped') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('ungroupped',true)">Da raggruppare</span><br/><em>N.</em><span class="gray right"><?php echo $numOfUngrouppedDDT; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'groupped') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('groupped',true)">Raggruppati</span><br/><em>N.</em><span class="gray right"><?php echo $numOfGrouppedDDT; ?></span></div></li>
	 <li class="<?php echo ($_REQUEST['show'] == 'internal') ? 'selected' : ''; ?>"><div><span class='title' onclick="showPage('internal',true)">Bolle movim. int.</span><br/><em>N.</em><span class="gray right"><?php echo $numOfIntDocs; ?></span></div></li>

	 <?php
	 /* SOTTOCATEGORIE */
	 $ret = GShell("dynarc cat-list -ap commercialdocs -pt `DDTIN` --get-total-items-count --order-by 'name ASC'",$_REQUEST['sessid'],$_REQUEST['shellid']);
	 if(!$ret['error'])
	 {
	  $list = $ret['outarr'];
	  for($c=0; $c < count($list); $c++)
	  {
	   echo "<li class='".((($catInfo['id'] == $list[$c]['id']) && ($_REQUEST['show'] == "category")) ? 'selected' : '')."'><div><span class='title' onclick='showCat("
		.$list[$c]['id'].")'>".$list[$c]['name']."</span><br/><em>Tot. DDT:</em><span class='gray right'>"
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['DDTIN'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['DDTIN'] : array(
	 		0 => array("tag"=>"name", "title"=>"D.D.T.", "width"=>190, "style"=>"text-align:left"),
			1 => array("tag"=>"subject_code", "title"=>"Cod.", "width"=>50, "style"=>"text-align:left"),
			2 => array("tag"=>"subject_name", "title"=>"Fornitore", "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$orderBy = "ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC");
	$limit = ($from ? $from : "0").",".$rpp;
	if($_REQUEST['show'] == "ungroupped")
	 $limit = "";
	

	$ret = GShell("dynarc item-list -ap `commercialdocs` ".(($_REQUEST['catid'] && ($_REQUEST['show'] == "category")) ? "-cat `".$catInfo['id']."`" : "-into DDTIN")." -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `"
		.$orderBy."`".($limit ? " -limit ".$limit : "")." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
	...
	if($_REQUEST['show'] == "ungroupped")
	{
	 ...
	 for($c=0; $c < count($list); $c++)
	 {
	  ...
	  //$ordList[$c] = $itm['subject_name'];
	  $ordList[$c] = $itm['subject_id'];
	 }
	...
	for($c=0; $c < count($list); $c++)
	{
	 ...
	 if(($lastMonth != date('n',$itm['ctime'])) || ($lastYear != date('Y',$itm['ctime'])))
	 {
	  ...
	  if($_REQUEST['show'] == "ungroupped")
	  {
	   //if($lastSubjectName && ($lastSubjectName != $itm['subject_name']))
	   if($lastSubjectId && ($lastSubjectId != $itm['subject_id']))
	    echo "<tr class='label2'><td colspan='100'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/lu.gif' style='margin-left:16px;'/> <i>Raggruppa tutti i D.D.T. selezionati per ".$lastSubjectName."</i><a href='#' onclick='groupDocs(this,true)' class='roundbtn'>Raggruppa tutti</a> <a href='#' class='roundbtn' onclick='groupDocs(this)'>Raggruppa selezionati</a></td></tr><tr class='label2'><td colspan='100'>&nbsp;</td></tr>";
	   $lastSubjectName = $itm['subject_name'];
	   $lastSubjectId = $itm['subject_id'];
	  }
	 ...
	 echo "<td><span class='subject'>".$itm['subject_code']."</span></td>";
	...
	if(($_REQUEST['show'] == "ungroupped") && count($list))
	{
	 echo "<tr class='label2'><td colspan='100'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/lu.gif' style='margin-left:16px;'/> <i>Raggruppa tutti i D.D.T. selezionati per ".$lastSubjectName."</i><a href='#' onclick='groupDocs(this,true)' class='roundbtn'>Raggruppa tutti</a> <a href='#' class='roundbtn' onclick='groupDocs(this)'>Raggruppa selezionati</a></td></tr><tr class='label2'><td colspan='100'>&nbsp;</td></tr>";
	}
	?>
	...
	if($_REQUEST['show'] == "ungroupped")
	{
	 $from = 1;
	 $to = $count;
	 $rpp = $serpInfo['resultsperpage'] = $count;
	}
	else
	{
 	 $rpp = 100;
...
function bodyOnLoad()
{
 ...
 var mE = EditSearch.init(document.getElementById('subject'),
	"dynarc search -ap `rubrica` -into vendors -fields name,code_str `","` -limit 10 --order-by 'name ASC'",
...
function nextPage()
{
 ...
 sh.OnOutput = function(o,a){
	 ...
	 if(a && a['items'])
	 {
	  ...
	  for(var c=0; c < a['items'].length; c++)
	  {
	   ...
	   r.insertCell(-1).innerHTML = "<span class='subject'>"+itm['subject_code']+"</span>";
	   ...
	   r.cells[5].style.width='100px';
	   r.insertCell(-1).innerHTML = "<b><em>&euro;</em> "+formatCurrency(itm['total'],2)+"</b>";
	   r.cells[6].style.textAlign='right'; r.cells[5].style.width='90px';*/
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` "+((show == "category") ? "-cat `"+CAT_ID+"`" : "-into DDTIN")+" -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function openDocument(id)
{
 ...
 window.open(ABSOLUTE_URL+"GCommercialDocs/docinfo.php?id="+id,"blank");
}
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -cat `"+CAT_ID+"` -group commdocs-ddtin");
}
...
function groupSelected()
{
 ...
 sh.sendCommand("commercialdocs group -type PURCHASEINVOICES"+q);
}
...
function groupDocs(btn,all)
{
 var labelR = btn.parentNode.parentNode;
 var tb = labelR.parentNode;
 var q = "";
 var count = 0;
 for(var c=labelR.rowIndex-1; c > -1; c--)
 {
  var r = tb.rows[c];
  if(r.cells[0].colSpan > 1)
  {
  ...
  if(r.id && (all || r.cells[0].getElementsByTagName('INPUT')[0].checked))
  {
   q+= " -id "+r.id;
   count++;
  }
 ...
 if(!count)
 {
  alert("Devi selezionare almeno un documento");
  return;
 }
 ...
 sh.OnOutput = function(o,a){
	 if(!a) return;
	 alert((all ? "Tutti i" : count)+" documenti sono stati raggruppati!");
	 openDocument(a['id']);
	}
 sh.sendCommand("commercialdocs group -type PURCHASEINVOICES"+q);
}
