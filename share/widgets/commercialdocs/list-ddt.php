...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: D.D.T. list
 #VERSION: 2.9beta
 #CHANGELOG: 26-10-2014 : Sostituito total con tot_netpay
			 26-05-2014 : Integrato nuovo sistema di gestione colonne
			 05-02-2014 : I DDT da raggruppare li ordina per id cliente.
			 04-02-2014 : Auto validazione del codice fiscale e della partita iva.
			 31-01-2014 : Bug fix su open-document.
			 ...
			 17-07-2013 : Aggiunto bolle di movim. interna.
...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['DDT'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['DDT'];
...
<tr><td valign='middle' class='title' width='200'><?php if($_REQUEST['show'] == "trash") echo "DDT cestinati"; else echo "Elenco dei D.D.T."; ?></td>
...
if(!$catInfo)
{
 $ret = GShell("dynarc cat-info -ap `commercialdocs` -tag DDT",$_REQUEST['sessid'],$_REQUEST['shellid']);
 if(!$ret['error'])
  $catInfo = $ret['outarr'];
}

$ret = GShell("dynarc item-list -ap `commercialdocs` -into ddt -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct ddt -where `conv_doc_id=0 AND group_doc_id=0` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfUngrouppedDDT = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct ddt -where `status=9` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfGrouppedDDT = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct ddt -where `tag='INTERNAL'` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 ...
	 <?php
	 /* SOTTOCATEGORIE */
	 $ret = GShell("dynarc cat-list -ap commercialdocs -pt `DDT` --get-total-items-count --order-by 'name ASC'",$_REQUEST['sessid'],$_REQUEST['shellid']);
	 if(!$ret['error'])
	 {
	  $list = $ret['outarr'];
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['DDT'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['DDT'] : array(
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` ".(($_REQUEST['catid'] && ($_REQUEST['show'] == "category")) ? "-cat `".$catInfo['id']."`" : "-into DDT")." -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `"
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` "+((show == "category") ? "-cat `"+CAT_ID+"`" : "-into DDT")+" -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -cat `"+CAT_ID+"` -group commdocs-ddt");
}
...
function groupSelected()
{
 ...
 sh.sendCommand("commercialdocs group"+q);
}
