...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 26-10-2014
 #PACKAGE: gcommercialdocs-paymentnotice
 #DESCRIPTION: Payment notice list
 #VERSION: 2.4beta
...
$_STATUSEXTRA = array();
if($_COMMERCIALDOCS_CONFIG['STATUSEXTRA'] && $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['PAYMENTNOTICE'])
 $_STATUSEXTRA = $_COMMERCIALDOCS_CONFIG['STATUSEXTRA']['PAYMENTNOTICE'];
...
<tr><td valign='middle' class='title' width='300'><?php if($_REQUEST['show'] == "trash") echo "Avvisi cestinati"; else echo "Elenco avvisi di pagamento"; ?></td>
	<td valign='middle' width='240'><ul class='basicbuttons'>
		 ...
		 if($_REQUEST['show'] == "trash")
		  ...
		  echo "<li><span href='#' onclick='newDoc()'><img src='".$_ABSOLUTE_URL."share/widgets/commercialdocs/img/add.gif' border='0'/> Crea un nuovo avviso di pag.</span></li>";
		 ?>
	</ul></td>
	<td>	 
...
<?php
$ret = GShell("dynarc cat-info -ap `commercialdocs` -tag paymentnotice",$_REQUEST['sessid'],$_REQUEST['shellid']);
$catInfo = $ret['outarr'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct paymentnotice -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfDocuments = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct paymentnotice -where `status=4` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfWorkingOrders = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct paymentnotice -where `status=5` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfSuspendedOrders = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct paymentnotice -where `status=6` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
$numOfFailedOrders = $ret['outarr']['count'];

$ret = GShell("dynarc item-list -ap `commercialdocs` -ct paymentnotice -where `status=7` -limit 1",$_REQUEST['sessid'],$_REQUEST['shellid']);
...
<tr><td valign='top'>
	<ul class='maintab' id='maintab'>
	 <li class="<?php echo (!$_REQUEST['show']) ? 'selected' : 'first'; ?>"><div><span class='title' onclick="showPage()">Tutti gli avvisi</span><br/><em>Tot. doc:</em><span class="gray right"><?php echo $numOfDocuments; ?></span></div></li>
...
<tr><td colspan='3' style="border-left:1px solid #dedede;border-bottom:1px solid #dedede;border-right:1px solid #dedede;border-collapse:collapse;">
	...
	<tr><th width='32'><input type='checkbox' onchange="selectAllRows(this)" id="tbselectall"/></th>
		<?php
		$_COLUMNS = $_COMMERCIALDOCS_CONFIG['COLUMNS']['PAYMENTNOTICE'] ? $_COMMERCIALDOCS_CONFIG['COLUMNS']['PAYMENTNOTICE'] : array(
	 		0 => array("tag"=>"name", "title"=>"Avviso", "width"=>190, "style"=>"text-align:left"),
...
<div id="itemlist-container" class="itemlist-container" style="height:<?php echo $_REQUEST['frameheight']-198; ?>px">
	...
	$ret = GShell("dynarc item-list -ap `commercialdocs` -ct paymentnotice -extget cdinfo".($where ? " -where `".ltrim($where," AND ")."`" : "")." --order-by `ctime ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC").",code_num ".($_REQUEST['sort']=='asc' ? "ASC" : "DESC")."` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info".($_REQUEST['show'] == "trash" ? " --include-trash" : ""),$_REQUEST['sessid'],$_REQUEST['shellid']);
	if(!$ret['error'])
	{
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `commercialdocs` -ct paymentnotice -extget cdinfo<?php echo $where ? ' -where `'.ltrim($where,' AND ').'`' : ''; ?> --order-by `ctime <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>,code_num <?php echo ($_REQUEST['sort']=='asc') ? 'ASC' : 'DESC'; ?>` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
...
function newDoc()
{
 ...
 sh.sendCommand("dynarc new-item -ap `commercialdocs` -ct paymentnotice -group commdocs-paymentnotice");
}
...
function emptyTrash()
{
 ...
 sh.sendCommand("dynarc trash empty -ap commercialdocs -ct paymentnotice");
}
