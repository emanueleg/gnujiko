...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #VERSION: 2.0beta
 #CHANGELOG:
 #TODO: Bisogna fare in modo che si possa filtrare ad esempio solo immagini (jpg,bmp,gif,png), così image-upload diventerebbe deprecato.
 #DEPENDS:
...
?>
<div class="fileupload-form-section">
...
<div class="fileupload-form-section" style="height:370px;overflow:auto;">

<?php
$ret = GShell("ls".($_REQUEST['path'] ? " `".$_REQUEST['path']."`" : ""));
...
{
 $dir = $dirs[$c];
 echo "<div class='block-dir' id='".fullescape($dir['path'])."'><img onclick='openDir(this)' src='".$_ABSOLUTE_URL."share/widgets/fileupload/img/folder.png'/><br/>";
...
for($c=0; $c < count($files); $c++)
{
 ...
 echo "<div class='block-img' id='".fullescape($file['path'])."' style=\"background-image:url('";
 if($icon && $icon['size22x22'])
  echo $_ABSOLUTE_URL.$icon['size48x48'];
 else
  echo $_ABSOLUTE_URL."share/mimetypes/48x48/file.png";
 echo "')\">";
...
<div class="fileupload-form-footer" style="height:42px;">
	<hr style='margin-top:0px;'/>
	<span class="button-blue" style="margin-left:10px;"><span onclick="submit()"><img src="<?php echo $_ABSOLUTE_URL; ?>share/widgets/fileupload/img/upload.png"/>Carica</span></span>
...
function submit()
{
 ...
 {
  var div = document.getElementById(SELECTED_FILES[c]);
  var icon = div.style.backgroundImage;
  icon = icon.replace('url("','');
