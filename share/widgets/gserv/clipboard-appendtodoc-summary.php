...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 10-07-2012
 #PACKAGE: gsderv
 #DESCRIPTION: Clipboard Summary for GServ
 #VERSION: 2.0beta
 #CHANGELOG:
 #TODO: La clipboard inserisce solo nell'archivio gserv. Modificare in modo che si possa usare anche con gli altri archivi.
...
<link rel="stylesheet" href="<?php echo $_ABSOLUTE_URL; ?>share/widgets/gserv/clipboard-appendtodoc-summary.css" type="text/css" />
<?php
...
<tr><td colspan="3" valign="top" class="contents"><div class="contents">
	 ...
	 <div class="doc-info">
	  <span class='bluebtn' style='width:220px;'><span class='bluebtn-title' id="document-name"><?php
		 ...
		 <ul class="submenu" id="document-types">
	 	  <li onclick="selectDocType('preemptives')"><img src="<?php echo $_ABSOLUTE_URL; ?>GCommercialDocs/img/invoice.png"/>Preventivo</li>
	 	  <li onclick="selectDocType('orders')"><img src="<?php echo $_ABSOLUTE_URL; ?>GCommercialDocs/img/invoice.png"/>Ordine</li>
	 	  <li onclick="selectDocType('vendororders')"><img src="<?php echo $_ABSOLUTE_URL; ?>GCommercialDocs/img/invoice.png"/>Ordine fornitore</li>
	 	  <li onclick="selectDocType('invoices')"><img src="<?php echo $_ABSOLUTE_URL; ?>GCommercialDocs/img/invoice.png"/>Fattura</li>
		 </ul></span>
	...
	for($c=0; $c < count($cbInfo['elements']); $c++)
	{
	 ...
	 echo "<tr refap='".$el['ap']."' refid='".$itm['id']."'><td><input type='checkbox'/></td>";
...
<tr><td class="footer-left" valign="top">
	 <ul class='basicbuttons' style="margin-left:15px;margin-top:4px;float:left;">
	  <li><span onclick='comeBack()'><img src="<?php echo $_ABSOLUTE_URL; ?>share/widgets/gserv/img/back.png" border='0'/>Torna indietro</span></li>
	 </ul>
	&nbsp;
	</td>
	<td class="footer-right" colspan="2" valign="top">
	 <ul class='basicbuttons' style="float:right;margin-top:4px;margin-right:5px;">
	  <li><span onclick='submit()'><img src="<?php echo $_ABSOLUTE_URL; ?>share/widgets/gserv/img/confirm.png" border='0'/>Conferma</span></li>
	 </ul>
	&nbsp;
	</td></tr>
...
function submit()
{
 ...
 if(!DOC_ID)
 {
  ...
  sh.sendCommand("dynarc new-item -ap `commercialdocs` -ct `"+DOC_TYPE+"` -extset `cdinfo."+(CUST_ID ? "subjectid="+CUST_ID : "subject='''"+CUST_NAME+"'''")+",pricelist='"+PLID+"'`");
  return;
 }
 else
 {
  ...
  for(var c=1; c < tb.O.rows.length; c++)
  {
   ...
   sh.sendCommand("dynarc edit-item -ap `commercialdocs` -id `"+DOC_ID+"` -extset `cdelements.type='"+type+"',refap='"+refap+"',refid='"+refid+"',code='"+code+"',name='''"+desc+"''',qty='"+qty+"',price='"+unitprice+"',vat='"+vat+"',pricelist='"+PLID+"'`");
  }
 }
}
