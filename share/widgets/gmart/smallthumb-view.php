...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 04-10-2013
 #PACKAGE: gmart
 #DESCRIPTION: Small thumbnails view for GMart
 #VERSION: 2.2beta
 #CHANGELOG: 04-10-2013 : 04-10-2013 : Bug fix, non calcolava lo sconto
...
?>
<link rel="stylesheet" href="<?php echo $_ABSOLUTE_URL; ?>share/widgets/gmart/smallthumb-view.css" type="text/css" />
<?php 
...
$ret = GShell("dynarc item-list -ap `".$_AP."`".($_REQUEST['catid'] ? " -cat `".$_REQUEST['catid']."`" : "")." -extget `gmart,thumbnails,coding,storeinfo,pricing`"
...
for($c=0; $c < count($list); $c++)
{
 ...
 if($item['thumbnails'][0] && ($_REQUEST['thumbmode'] != "never"))
  ...
  echo "<div class='thumbnail' style='background-image: url(".$_ABSOLUTE_URL."share/widgets/gmart/img/photo.png);'>";
 else
	 ...
	 if(a && a['items'])
	 {
	  for(var c=0; c < a['items'].length; c++)
	  {
	   ...
	   if(item['thumbnails'] && item['thumbnails'][0] && (THUMBMODE != "never"))
		...
		html+= "<div class='thumbnail' style='background-image: url("+ABSOLUTE_URL+"share/widgets/gmart/img/photo.png);'>";
	   else
