...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 24-07-2012
 #PACKAGE: gmart
 #DESCRIPTION: Thumbnails view for GMart
 #VERSION: 2.1beta
 #CHANGELOG: 24-07-2013 : Modifiche varie.
 #TODO:
...
?>
<link rel="stylesheet" href="<?php echo $_ABSOLUTE_URL; ?>share/widgets/gmart/thumbnails-view.css" type="text/css" />

<?php 
$_AP = $_REQUEST['ap'] ? $_REQUEST['ap'] : "gmart";
$rpp = $_REQUEST['limit'] ? $_REQUEST['limit'] : 50;
$from = $_REQUEST['pg'] ? ($rpp*($_REQUEST['pg']-1)) : 0;
$ret = GShell("dynarc item-list -ap `".$_AP."`".($_REQUEST['catid'] ? " -cat `".$_REQUEST['catid']."`" : "")." -extget `gmart,thumbnails,coding,storeinfo` -limit ".($from ? $from : "0").",".$rpp." --return-serp-info",$_REQUEST['sessid'],$_REQUEST['shellid']);
...
function nextPage()
{
 ...
 sh.sendCommand("dynarc item-list -ap `"+AP+"`"+(CAT_ID ? " -cat `"+CAT_ID+"`" : "")+" -extget `gmart,thumbnails,coding,storeinfo` -limit "+(RESULTS_PER_PAGE*CURRENT_PAGE)+","+RESULTS_PER_PAGE);
}
