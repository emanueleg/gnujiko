...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 27-08-2014
 #PACKAGE: backoffice2
 #DESCRIPTION: BackOffice 2 - Scadenziario
 #VERSION: 2.3beta
 #CHANGELOG: 27-08-2014 : restricted access integration.
			 24-05-2014 : Bug fix.
			 27-02-2014 : Fatto traduzioni.
 #TODO:
...
$centerContents = "<input type='text' class='contact' style='width:400px;float:left' placeholder='".i18n('Filter by customer...')."' id='search' value=\"".htmlspecialchars($_REQUEST['search'],ENT_QUOTES)."\" modal='extended' fields='code_str,name' contactfields='phone,phone2,cell,email'/><input type='button' class='button-search' id='searchbtn'/>";
...
?>
 ...
 <td width='120'><input type='button' class="button-blue menuwhite" value='Menu' connect='mainmenu' id='menubutton'/>
		<ul class='popupmenu' id='mainmenu'>
		 <li onclick='generateRiBa()'><img src="<?php echo $_ABSOLUTE_URL; ?>share/icons/16x16/new_file.png"/> <?php echo i18n("Generate Ri.Ba."); ?></li>
		 ...
		 <!-- <li onclick='generateSollecito()'>Invia sollecito di pagamento</li> -->
		</ul></td>
...
$cmd = "backoffice get-incomes-schedule -from '".$dateFrom."' -to '".$dateTo."' --only-invoices";
...
<tr><th width='16' noprint='true'><input type='checkbox'/></th>
	...
	<th width='100' colwidth='25'><?php echo i18n("Document"); ?></th>
	...
	<th sortable='true' field='subject_name' colwidth='40'><?php echo i18n("Customer"); ?></th>
...
function generateRiBa()
{
 ...
 document.location.href = "generateriba.php?ids="+q.substr(1);
}

function generateSollecito()
{
...
{
 var doc = new GnujikoPrintableDocument("<?php echo i18n('Schedule'); ?>", "A4");
 ...
 var header = "<div style='width:190mm' class='defaultheader'><h3><?php echo i18n('Schedule'); ?> - <?php echo i18n('from'); ?> "+dateFrom.printf('d/m/Y')+" <?php echo i18n('to'); ?> "+dateTo.printf('d/m/Y')+"</h3></div>";
