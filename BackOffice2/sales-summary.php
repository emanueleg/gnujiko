...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 20-04-2017
 #PACKAGE: backoffice2
 #DESCRIPTION: BackOffice 2 - Riepilogo vendite
 #VERSION: 2.4beta
 #CHANGELOG: 20-04-2017 : Aggiunta colonna guadagno.
			 27-03-2015 : Bug fix su funzione ExportToExcel (problemi con asterischi).
			 21-03-2015 : Aggiunta funzione esporta su Excel.
...
$template->Begin(i18n('Sales summary'));
...
$cmd = "backoffice sales-summary -from '".date('Y-m-d',$dateFrom)."' -to '".date('Y-m-d',$dateTo)." 23:59:59'";
if($_AP) $cmd.= " -ap '".$_AP."'";
if($_ID) $cmd.= " -id '".$_ID."'";
...
?>
 ...
 </td>-->	<th width='80' style='text-align:center' colwidth='25'><?php echo i18n('Amount'); ?></th>
 <td>&nbsp;
...
<div class="totals-footer">
 <table width="100%" cellspacing="0" cellpadding="0" border="0">
  <tr><td rowspan='2' valign='middle'>
	  ...
	  <td align='right'><span class='smalltext'><?php echo i18n('Guadagno'); ?></span></td></tr>
  <tr><td align='center'><span class='smalltext' id='foot-totqty'><?php echo $_TOT_QTY; ?></span></td>
	  ...
	  <td align='right'><span class='bigtext' id='foot-totprofit'><b><?php echo number_format($_TOT_PROFIT,2,',','.'); ?> &euro;</b></span></td></tr>
 </table>
</div>
...
function Print(printBtn)
{
 ...
 var doc = new GnujikoPrintableDocument("<?php echo i18n('Sales summary'); ?>", "A4");
 var header = "<div style='width:190mm' class='defaultheader'><h3><?php echo i18n('Sales summary'); ?> - <?php echo i18n('from'); ?> "+dateFrom.printf('d/m/Y')+" <?php echo i18n('to'); ?> "+dateTo.printf('d/m/Y')+"</h3></div>";
...
function ExportToExcel(btn)
{
 ...
 /* Rimosso per problemi con asterischi */
 //cmd+= " | gframe -f excel/export -params `file=riepilogo_vendite` --use-cache-contents -c";
 ...
 sh.OnOutput = function(o,a){
	 ...
	 sh2.sendCommand("gframe -f excel/export -params `file=riepilogo_vendite` --use-cache-contents -c `"+o+"`");
	}
