...
{
	global $Config, $_DEFAULT_FILE_PERMS ;
	...
	if ( !file_exists( $folderPath ) )
	{
		...
		{
		 if(defined("GNUJIKO_FTP_USERNAME"))
		 {
		  // crea la cartella usando FTP //
		  $conn = ftp_connect($_SERVER['SERVER_NAME']);
		  if(ftp_login($conn,GNUJIKO_FTP_USERNAME,GNUJIKO_FTP_PASSWORD))
  		  {
		   if(defined("GNUJIKO_FTP_PATH"))
		   {
		    $fldPath = str_replace("../","",$folderPath);
		    $fldPath = GNUJIKO_FTP_PATH.$fldPath;
		   }
		   else
		    $fldPath = $folderPath;
		   ftp_mkdir($conn, $fldPath);
		  }
		 }
		 else
		  mkdir( $folderPath ) ;
		...
		{
			$permissions = $_DEFAULT_FILE_PERMS ;
		 ...
		 if(defined("GNUJIKO_FTP_USERNAME"))
		 {
		  ...
		  if(ftp_login($conn,GNUJIKO_FTP_USERNAME,GNUJIKO_FTP_PASSWORD))
		   ...
		   if(defined("GNUJIKO_FTP_PATH"))
		   {
		    ...
		    $fldPath = $folderPath;
		   ftp_mkdir($conn, $fldPath);
		   ftp_chmod($conn, $permissions, $fldPath);
		  }
		 }
		 else
		 {
