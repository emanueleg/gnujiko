...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class HTML2PDF
    {
        ...
        public function writeHTML($html, $debugVue = false)
        {
			...
			$steps = count($this->parsingHtml->code);
			if($steps)
			{
...
'.$content.'
		 ...
		 global $_SESSID, $_SHELLID, $_PREOFF, $_CHUNK_SIZE, $_PROGRESS, $_BYPASS_PREOUTPUT;
            ...
            for ($this->_parsePos=0; $this->_parsePos<count($this->parsingHtml->code); $this->_parsePos++) {
				if(!$_PREOFF && !$_BYPASS_PREOUTPUT)
				{
				 ...
				 {
				  gshPreOutput($_SHELLID,"Please wait, I'm converting the document to pdf", "PROGRESS", "");
				  $_PROGRESS = 0;
				 }
				 $_PREON = true;
				 $_PREOFF = true;
		...
		protected function _tag_open_COLGROUP($params)
		{
		 return true;
		}

		protected function _tag_close_COLGROUP()
		{
		 return true;
		}
