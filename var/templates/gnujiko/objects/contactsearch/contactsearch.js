/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: Gnujiko Template - Contact search object
 #VERSION: 2.1beta
 #CHANGELOG: 21-05-2017 : Aggiunto parametro agentid.
 #TODO:
 ...
 */
 ...
 this.agentId = ed.getAttribute('agentid') ? ed.getAttribute('agentid') : "";
 this.qryTimer = null;
 ...
 this.O.className = "slcontactsearch-container";

 this.ULDIV = document.createElement('DIV');
 this.ULDIV.className = "slcontactsearch-menu-div"
 this.UL = document.createElement('UL');
 this.UL.className = "slcontactsearch-menu";

 this.Optbox = document.createElement('DIV');
 this.Optbox.className = "slcontactsearch-optbox";
...
GLContactSearch.prototype.initOptbox = function()
{
 ...
 var html = "<div class='slcontactsearch-optbox-header'>";
 ...
 html+= "<div class='slcontactsearch-optbox-body' style='display:none'>";
 html+= "<table width='100%' cellspacing='0' cellpadding='0' border='0'>";
 html+= "<tr><td width='70' align='center' valign='middle'><img src='"+ABSOLUTE_URL+"var/templates/gnujiko/objects/contactsearch/img/search-opt.png'/></td>";
 ...
 html+= "</div></td>";
 html+= "<td width='70' valign='middle' align='center' style='border-left:1px solid #e6e6e6'><img src='"+ABSOLUTE_URL+"var/templates/gnujiko/objects/contactsearch/img/repeat-query.png'/><br/><span class='optbox-repeatqry-link'>Ripeti la ricerca</span></td></tr>";
 html+= "</table>";
 html+= "</div>"; // eof - slcontactsearch-optbox-body
...
GLContactSearch.prototype.execQry = function()
{
 ...
 if(this.agentId)		cmd+= " -agentid '"+this.agentId+"'";
...
GLContactSearch.prototype.addResult = function(data)
{
 ...
 if(this.modal == "extended")
  var html = "<div class='slcontextinfo-left' style='width:"+(this.O.offsetWidth-150)+"px'>";
 else
  var html = "<div class='slcontextinfo-left' style='width:"+(this.O.offsetWidth-50)+"px'>";

  html+= "<div class='slcontact-name'><span class='slcontact-code'>"+data['code_str']+"</span> - "+data['name']+"</div>";
  var addr = "";
  if(data['contacts'])
   addr = data['contacts'][0]['address']+" "+data['contacts'][0]['city'];
  html+= "<div class='slcontact-address'>"+addr+"</div>";
 html+= "</div>";

 if(this.modal == "extended")
 {
  html+= "<div class='slcontextinfo-right'>";
  var phone = "";
  var email = "";
  if(data['contacts'])
  {
  ...
  html+= "<div class='slcontact-phone'>"+phone+"</div>";
  html+= "<div class='slcontact-email'>"+email+"</div>";
  html+= "</div>";
 }
...
//--------------------------------------------------------------------------------------------------------------------//
//--------------------------------------------------------------------------------------------------------------------//
var ACTIVE_SLCONTACTSEARCH = null;
function hideActiveGLContactSearch()
{
 if(ACTIVE_SLCONTACTSEARCH)
  ACTIVE_SLCONTACTSEARCH.hide();
}
