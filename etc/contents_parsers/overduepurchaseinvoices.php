...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DESCRIPTION: Overdue purchase invoices parser.
 #VERSION: 2.0beta
 #CHANGELOG: 
 #TODO:
...
function gnujikocontentparser_overduepurchaseinvoices_info($sessid, $shellid)
{
...
function gnujikocontentparser_overduepurchaseinvoices_parse($contents, $params, $sessid, $shellid)
{
 global $_ABSOLUTE_URL, $_BASE_PATH;

 $ret = GShell("backoffice get-expenses-schedule -from lastmonth -to today --only-invoices",$sessid,$shellid);
 ...
 for($c=0; $c < count($list); $c++)
 {
  ...
  if($item['expired'])
  {
   if(!$_DOCUMENTS[$item['docinfo']['id']])
   {
	$a = array('id'=>$item['docinfo']['id'], 'name'=>$item['docinfo']['name'], 'subject_id'=>$item['docinfo']['subject_id'], 'subject_name'=>$item['docinfo']['subject_name'], 'amount'=>$item['amount'], 'expire_date'=>$item['expire_date'], 'payment_mode'=>$item['docinfo']['payment_mode'], 'payment_mode_name'=>$item['docinfo']['payment_mode_name']);
	// get doc info //
	$db->RunQuery("SELECT * FROM dynarc_commercialdocs_items WHERE id='".$item['docinfo']['id']."'");
	$db->Read();
