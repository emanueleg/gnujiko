...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 #DATE: 16-03-2017
 #PACKAGE: stats
 #DESCRIPTION: Orders statistics service.
 #VERSION: 2.0beta
 #CHANGELOG: 
 #TODO: 
...
*/
//-------------------------------------------------------------------------------------------------------------------//
function gnujikostatservice_orders_info($sessid,$shellid)
{
 $info = array(
	 "name"=>"Statistiche ordini evasi"
	);
 return $info;
}
//-------------------------------------------------------------------------------------------------------------------//
function gnujikostatservice_orders_enable($sessid,$shellid)
{
}
//-------------------------------------------------------------------------------------------------------------------//
function gnujikostatservice_orders_disable($sessid,$shellid)
{

}
//-------------------------------------------------------------------------------------------------------------------//
function gnujikostatservice_orders_makeTables($year)
{
 global $_DATABASE_NAME;

 $db = new AlpaDatabase();
 $qry = "CREATE TABLE IF NOT EXISTS `stats_prodsoldbyorder_".$year."` (
 ...
 $ret = $db->RunQuery("CREATE TABLE IF NOT EXISTS `stats_prodsoldbyorder_daily_".$year."` (
 `id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
 `date` DATE NOT NULL ,
 `ref_ap` VARCHAR(32) NOT NULL ,
 `ref_id` INT(11) NOT NULL ,
 ...
 $ret = $db->RunQuery("CREATE TABLE IF NOT EXISTS `stats_prodsoldbyorder_indexes` (
 `id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY ,
...
//-------------------------------------------------------------------------------------------------------------------//
function gnujikostatservice_orders_makeIndex($args, $sessid, $shellid)
{
 ...
 $out.= "Make index of orders from ".date('d/m/Y',$dateFrom)." to ".date('d/m/Y',$dateTo)." ...";
 ...
 /* Empty daily tables */
 $db = new AlpaDatabase();
 $db->RunQuery("DELETE FROM stats_prodsoldbyorder_daily_".$year." WHERE date>='".date('Y-m-d',$dateFrom)."' AND date<'".date('Y-m-d',$dateTo)."'");
 $db->RunQuery("DELETE FROM stats_prodsoldbyorder_daily_".$toYear." WHERE date>='".date('Y-m-d',$dateFrom)."' AND date<'".date('Y-m-d',$dateTo)."'");
 $db->Close();

 /* CHECK OR MAKE TABLES */
 $ret = gnujikostatservice_orders_makeTables($year);
 if($ret['error']) return $ret;
 if($toYear != $year)
 {
  $ret = gnujikostatservice_orders_makeTables($toYear);
  if($ret['error']) return $ret;
 }

 /* GET CATEGORIES */
 $catTags = array("ORDERS");
 $catIds = array();
 $catQry = "";
 $db = new AlpaDatabase();
 ...
 $query.= " AND (".ltrim($catQry," OR ").") AND trash='0' AND status>=7";
 ...
 while(list($date,$arr)=each($data))
 {
  ...
  while(list($ap,$items)=each($arr))
  {
   ...
   {
    $db->RunQuery("INSERT INTO stats_prodsoldbyorder_daily_".date('Y',strtotime($date))."(date,ref_ap,ref_id,ref_cat,ref_sec,qty,amount) VALUES('"
 ...
 while(list($year,$arr)=each($data2))
 {
  ...
  while(list($ap,$items)=each($arr))
  {
   ...
   {
	reset($info['months']);
    $qry = "INSERT INTO stats_prodsoldbyorder_".$year."(ref_ap,ref_id,ref_cat,ref_sec";
 ...
 $db = new AlpaDatabase();
 $db->RunQuery("SELECT id,date_from FROM stats_prodsoldbyorder_indexes WHERE date_from<='".$dFrom."' AND date_to>'".$dFrom."' AND date_to<='".$dTo."' LIMIT 1");
 if($db->Read())
 {
  $leftId = $db->record['id'];
  $dFrom = date('Y-m-d',strtotime($db->record['date_from']));
  $db->RunQuery("DELETE FROM stats_prodsoldbyorder_indexes WHERE id='".$leftId."'");
 }
 $db->RunQuery("SELECT id,date_to FROM stats_prodsoldbyorder_indexes WHERE date_from>='".$dFrom."' AND date_from<'".$dTo."' AND date_to>'".$dTo."' LIMIT 1");
 if($db->Read())
 {
  $rightId = $db->record['id'];
  $dTo = date('Y-m-d',strtotime($db->record['date_to']));
  $db->RunQuery("DELETE FROM stats_prodsoldbyorder_indexes WHERE id='".$rightId."'");
 }

 // rimuovo tutti gli indici che hanno un range di date all'interno di questo
 $db->RunQuery("DELETE FROM stats_prodsoldbyorder_indexes WHERE date_from>'".$dFrom."' AND date_to<'".$dTo."'");

 $db->RunQuery("SELECT id FROM stats_prodsoldbyorder_indexes WHERE date_from>='".$dFrom."' AND date_to<='".$dTo."' LIMIT 1");
 if($db->Read())
  $db->RunQuery("UPDATE stats_prodsoldbyorder_indexes SET mtime='".$now."',date_from='".$dFrom."',date_to='".$dTo."' WHERE id='".$db->record['id']."'");
 else
  $db->RunQuery("INSERT INTO stats_prodsoldbyorder_indexes(mtime,date_from,date_to) VALUES('".$now."','".$dFrom."','".$dTo."')");
 $db->Close();
...
//-------------------------------------------------------------------------------------------------------------------//
function gnujikostatservice_orders_get($args, $sessid, $shellid)
{
 global $_BASE_PATH;
 $out = "";
 $outArr = array();
 ...
 $out.= "Get statistics of orders from ".date('d/m/Y',$dateFrom)." to ".date('d/m/Y',$dateTo)." ...";
 $year = date('Y',$dateFrom);
 $toYear = date('Y',$dateTo);
 if($toYear > ($year+1))
 ...
 $stats = new GnujikoStats("prodsoldbyorder");
