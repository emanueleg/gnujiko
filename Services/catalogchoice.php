...
<tr><td valign='top'>
	<div class="catalog-container" id="catalogcontainer">
	 ...
	 for($c=0; $c < count($_CATALOGS); $c++)
	 {
	  ...
	  echo "<div id='".$_CATALOGS[$c]['id']."' class='bigcatalog ".($_CATALOGS[$c]['params']['gserv-theme'] ? $_CATALOGS[$c]['params']['gserv-theme'] : "light-green")."'>";
	  echo "<div class='headtit'>CATALOGO<a class='editbtn' href='#' onclick='editCatalog(".$_CATALOGS[$c]['id'].")'><img src='".$_ABSOLUTE_URL."Services/img/edit.png' border='0' title='Modifica'/></a></div>";
...
{
 document.location.href = ABSOLUTE_URL+"Services/index.php?aid="+id;
}
...
function editCatalog(id)
{
 ...
 sh.sendCommand("gframe -f gserv/edit.catalog -params `id="+id+"`");
}
...
function newCatalog()
{
 ...
 sh.sendSudoCommand("dynarc archive-list -type gserv -a --ret-count || dynarc new-archive -name `"+_name+"` -prefix 'gserv_'+*.count -group gserv -type gserv --default-cat-perms 660 --default-item-perms 660 -inherit gserv --hidden");
}
